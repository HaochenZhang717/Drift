...
from models.interpretable_diffusion_flowts.model_utils import LearnablePositionalEncoding, Conv_MLP,\
...
class FullAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if int(os.environ.get('hucfg_attention_rope_use', '-1')) == 1: 
            freqs_cis = self.freqs_cis.to(device=q.device)[0 : T]
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, encoder_output, mask=None):
        ...
        freqs_cis = self.freqs_cis.to(k.device)[0 : T]
        q, k = apply_rotary_emb(q.permute(0,2,1,3), k.permute(0,2,1,3), freqs_cis=freqs_cis)
        q, k = q.permute(0,2,1,3), k.permute(0,2,1,3)
