...
DATASETS="${DATASETS:-household}"
LENGTHS="${LENGTHS:-64}"
GPU="${GPU:-0}"
DEVICE="${DEVICE:-cpu}"
DRY_RUN="${DRY_RUN:-0}"

GLUCOSE_DATA_DIR="${GLUCOSE_DATA_DIR:-/Users/zhc/Downloads/AI-READI-processed}"
LOCAL_TS_DATA_DIR="${LOCAL_TS_DATA_DIR:-/Users/zhc/Documents/Time_Series_Datasets}"

VQ_BATCH_SIZE="${VQ_BATCH_SIZE:-2}"
FLOW_BATCH_SIZE="${FLOW_BATCH_SIZE:-2}"

VQ_TOTAL_ITER="${VQ_TOTAL_ITER:-1}"
VQ_WARM_UP_ITER="${VQ_WARM_UP_ITER:-0}"
VQ_LR="${VQ_LR:-1e-4}"
VQ_LR_SCHEDULER="${VQ_LR_SCHEDULER:-200000}"
VQ_EVAL_ITER="${VQ_EVAL_ITER:-999999}"
VQ_PRINT_ITER="${VQ_PRINT_ITER:-1}"
VQ_WIDTH="${VQ_WIDTH:-32}"
VQ_CODE_DIM="${VQ_CODE_DIM:-16}"
VQ_NB_CODE="${VQ_NB_CODE:-32}"
...
FLOW_D_MODEL="${FLOW_D_MODEL:-32}"
FLOW_N_LAYERS="${FLOW_N_LAYERS:-1}"
FLOW_NUM_HEADS="${FLOW_NUM_HEADS:-4}"
...
FLOW_PRINT_INTERVAL="${FLOW_PRINT_INTERVAL:-1}"
FLOW_EVAL_INTERVAL="${FLOW_EVAL_INTERVAL:-999999}"
FLOW_EVAL_STEPS="${FLOW_EVAL_STEPS:-2}"
FLOW_MAX_ITERS="${FLOW_MAX_ITERS:-1}"
FLOW_NUM_SAMPLES="${FLOW_NUM_SAMPLES:-4}"
FLOW_SAMPLE_BATCH_SIZE="${FLOW_SAMPLE_BATCH_SIZE:-2}"
