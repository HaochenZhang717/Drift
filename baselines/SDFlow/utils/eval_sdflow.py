...
@torch.no_grad()
def evaluation_vqvae(args,out_dir, val_loader, net, logger, writer, nb_iter, best_iter, best_ds, best_mse, save = True, isTest = False, device = None) :
    net.eval()
    if discriminative_score_metrics is None:
        raise ModuleNotFoundError("TensorFlow metric dependencies are required for VQ-VAE evaluation. Use --skip-eval for smoke tests.")
    if device is None:
    ...
    if ds < best_ds :
        ...
        if save:
            torch.save({'net' : net.state_dict(), 'args': vars(args)}, os.path.join(out_dir, 'net_best_ds.pth'))
    ...
    if mse < best_mse :
        ...
        best_mse = mse
        if save:
            torch.save({'net' : net.state_dict(), 'args': vars(args)}, os.path.join(out_dir, 'net_best_mse.pth'))
