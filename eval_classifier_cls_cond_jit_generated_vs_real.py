...
def build_dataset(args: argparse.Namespace) -> AIREADIModalityImputationDataset:
    # Match multimodal-JiT eval filters so comparison is fair.
...
def build_cls_cond_jit_from_ckpt(args: argparse.Namespace, device: torch.device) -> Denoiser:
    ckpt = torch.load(args.cls_ckpt_path, map_location="cpu", weights_only=False)
    ckpt_args = dict(ckpt.get("args", {}))
    if not ckpt_args:
        raise ValueError("Checkpoint missing 'args'; cannot reconstruct cls_cond_jit model.")
    ...
    which = args.cls_weights
    if which not in {"model", "ema1", "ema2"}:
        raise ValueError("--cls_weights must be one of: model, ema1, ema2")
    ...
    model.eval()
    print(f"Loaded cls_cond_jit weights '{which}' from {args.cls_ckpt_path}")
    return model
...
def main(args: argparse.Namespace) -> None:
    ...
    for repeat_idx in range(args.num_repeats):
        ...
        print(f"Collected real: total={y_real.numel()} | per_class={k_real}")
    ...
    summary = {
        "settings": {
            "cls_ckpt_path": args.cls_ckpt_path,
    ...
    print("\n=== Repeated CLS-cond-JiT Classifier Quality Gap Summary ===")
    print(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate cls_cond_jit generated-vs-real quality with evaluator classifier")
    ...
    parser.add_argument("--cls_weights", type=str, default="ema2", choices=["model", "ema1", "ema2"])
    ...
    parser.add_argument("--output_json", type=str, default="cls_cond_jit_generated_vs_real_classifier_gap.json")
