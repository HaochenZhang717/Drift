...
from models.jepas.mmd_jepa_1.model_util import VisionRotaryEmbeddingFast, get_2d_sincos_pos_embed, RMSNorm
...
class ModalityEmbedder(nn.Module):
    """
    Embeds missing-modality labels into vector representations.
    """
    def __init__(self, num_modalities, hidden_size):
        super().__init__()
        self.embedding_table = nn.Embedding(num_modalities, hidden_size)
        self.num_modalities = num_modalities

    def forward(self, modality_labels):
        return self.embedding_table(modality_labels)


def scaled_dot_product_attention(query, key, value, dropout_p=0.0) -> torch.Tensor:
    L, S = query.size(-2), key.size(-2)
    scale_factor = 1 / math.sqrt(query.size(-1))
    attn_bias = torch.zeros(query.size(0), 1, L, S, dtype=query.dtype).to(device=query.device)
...
class JiT(nn.Module):
    ...
    def __init__(
        ...
        self.modality_embedder = ModalityEmbedder(num_modalities, hidden_size)
    ...
    def initialize_weights(self):
        ...
        nn.init.normal_(self.modality_embedder.embedding_table.weight, std=0.02)
    ...
    def forward(self, x, t, label, cond_tokens, missing_modality=None):
        """
        x: (N, C, H, W)
        t: time embedding: (N,)
        label: missing modality: (N,)
        cond_tokens: multimodal_condition_tokens: (N, seq_len, condition_dim)
        missing_modality: modality identity for AdaLN conditioning, (N,)
        """
        ...
        if missing_modality is not None:
            c = c + self.modality_embedder(missing_modality)
...
def JiT_B_2_Tiny(**kwargs):
    return JiT(depth=4, hidden_size=128, num_heads=4,
               bottleneck_dim=64, in_context_start=0, patch_size=2, **kwargs)
