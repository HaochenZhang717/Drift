...
from models.multimodal_jit.model_util import VisionRotaryEmbeddingFast, get_2d_sincos_pos_embed, RMSNorm
...
class JiT(nn.Module):
    ...
    def forward(self, x, t, label, cond_tokens, condition_embed=None):
        """
        x: (N, C, H, W)
        t: time embedding: (N,)
        label: study_group
        cond_tokens: multimodal_condition_tokens: (N, seq_len, condition_dim)
        condition_embed: optional extra condition vector: (N, hidden_size)
        """
