...
class DiTBlockCrossAttention(nn.Module):
    ...
    def forward(
        ...
        # === MLP ===
        main = main + self.mlp(self.norm3(main))
...
class DriftDiT(nn.Module):
    ...
    def __init__(
        ...
        self.ts_encoder = nn.Sequential(
            nn.Conv1d(in_channels, hidden_size // 4, kernel_size=3, padding=1, stride=2),
            nn.GroupNorm(8, hidden_size // 4),
            nn.GELU(),

            nn.Conv1d(hidden_size // 4, hidden_size // 2, kernel_size=3, padding=1, stride=2),
            nn.GroupNorm(8, hidden_size // 2),
            nn.GELU(),

            nn.Conv1d(hidden_size // 2, hidden_size, kernel_size=3, padding=1, stride=2),
    ...
    def forward(
        self,
        x: torch.Tensor,
        coarse_time_series: torch.Tensor,
    ) -> torch.Tensor:
        """
        Forward pass.
        ...
        ts_embed = self.ts_encoder(coarse_time_series)
