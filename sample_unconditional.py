...
def generate_grid(
    ...
    """
    Generate an unconditional sample grid.
    ...
    Args:
        ...
        img_size: Image size
        device: Device to generate on
        num_samples: Number of samples in the grid
        alpha: Drift alpha

    Returns:
        Grid samples, shape (num_samples, in_channels, img_size, img_size)
    """
...
def generate_alpha_sweep(
    ...
    labels = torch.zeros(samples_per_alpha, device=device, dtype=torch.long)

    samples = []
    for alpha in alphas:
        alpha_tensor = torch.full((samples_per_alpha,), alpha, device=device)
        x = model(z, labels, alpha_tensor)
        samples.append(x)
...
def sample_and_save(
    ...
    # Generate unconditional grid
    print(f"Generating unconditional grid ({num_samples} samples)...")
    grid = generate_grid(
        model,
        in_channels,
        img_size,
    ...
    grid_path = output_dir / f"grid_alpha{alpha:.1f}.png"
    grid_nrow = int(np.ceil(np.sqrt(num_samples)))
    save_image_grid(grid, str(grid_path), nrow=grid_nrow)
    print(f"Saved unconditional grid to {grid_path}")
    ...
    sweep_path = output_dir / "alpha_sweep_unconditional.png"
    save_image_grid(sweep_samples, str(sweep_path), nrow=8)
    print(f"Saved unconditional alpha sweep to {sweep_path}")
