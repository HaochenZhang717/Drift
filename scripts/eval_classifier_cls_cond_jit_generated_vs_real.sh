...
CLS_CKPT_DIR=/mnt/unites8/playpen/haochenz/Drift/outputs/cls_cond_jit_run1
CLF_CKPT=/playpen-shared/haochenz/Drift/outputs/aireadi_eval_classifier/best_classifier.pt
OUTPUT_DIR=/mnt/unites8/playpen/haochenz/Drift/outputs/aireadi_eval_classifier/cls_cond_jit_ckpt_sweep
...
for ckpt_name in "${CHECKPOINTS[@]}"; do
  ckpt_path="${CLS_CKPT_DIR}/${ckpt_name}"
  if [[ ! -f "${ckpt_path}" ]]; then
  ...
  for cls_weights in "${EMA_WEIGHTS[@]}"; do
    out_json="cls_cond_jit_gap_${ckpt_stem}_${cls_weights}.json"

    python eval_classifier_cls_cond_jit_generated_vs_real.py \
      --data_root "${DATA_ROOT}" \
      --participants_tsv_path "${PARTICIPANTS_TSV}" \
      --real_split test \
      \
      --cls_ckpt_path "${ckpt_path}" \
