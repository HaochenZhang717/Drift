...
from models.multimodal_jit_v2.denoiser import Denoiser
...
def build_dataset(args: argparse.Namespace, split: str) -> AIREADIModalityImputationDataset:
    ...
    return AIREADIModalityImputationDataset(
        ...
        cgm_enhanced_features_path=args.cgm_enhanced_features_path,
...
def batch_to_model_inputs(
    ...
    return {
        ...
        "cgm_enhanced_numeric_values": batch["cgm_enhanced_numeric_values"][valid].float().to(device),
        "cgm_enhanced_numeric_mask": batch["cgm_enhanced_numeric_mask"][valid].float().to(device),
        "cgm_enhanced_binary_values": batch["cgm_enhanced_binary_values"][valid].float().to(device),
        "cgm_enhanced_binary_mask": batch["cgm_enhanced_binary_mask"][valid].float().to(device),
        "cgm_enhanced_medication_codes": batch["cgm_enhanced_medication_codes"][valid].long().to(device),
        "cgm_enhanced_medication_mask": batch["cgm_enhanced_medication_mask"][valid].float().to(device),
    }


def build_model_args(args: argparse.Namespace) -> SimpleNamespace:
    has_all_ae_ckpts = all(
        getattr(args, name) is not None
        for name in [
            "ckpt_heart_rate",
            "ckpt_calorie",
            "ckpt_physical_activity",
            "ckpt_respiratory_rate",
        ]
    )
    ae_ckpt_paths = (
        {
    ...
    return SimpleNamespace(
        ...
        ae_cpt_paths=ae_ckpt_paths,
        ae_strict_load=args.ae_strict_load,
        ...
        cgm_medication_vocab_sizes=args.cgm_medication_vocab_sizes,
    )
...
def train(args: argparse.Namespace) -> None:
    ...
    if model_args.ae_cpt_paths is None:
        print("Warning: AE checkpoints are not provided. MultiModalEncoder AEs will run with random initialization.")
        ...
        def evaluate_eval_loss(eval_model: torch.nn.Module) -> float:
            ...
            with torch.no_grad():
                for batch in val_loader:
                    ...
                    val_steps_local += 1
                    if args.max_val_steps is not None and val_steps_local >= args.max_val_steps:
                        break
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train multimodal JiT v2 denoiser on AI-READI modality-imputation dataset")

    parser.add_argument("--data_root", type=str, default="./AI-READI")
    parser.add_argument("--participants_tsv_path", type=str, default=None)
    parser.add_argument("--cgm_enhanced_features_path", type=str, default=None)
    ...
    parser.add_argument("--ae_strict_load", action="store_true", default=False)
